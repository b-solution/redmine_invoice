class ClientQuery < Query
  ...
  self.available_columns = [
      QueryColumn.new(:name, :sortable => "#{Client.table_name}.subject",:groupable => true),
      QueryColumn.new(:address, :sortable => "#{Client.table_name}.location_online",:groupable => true, caption: 'location'),
      QueryColumn.new(:date, :sortable => "#{Client.table_name}.date",:groupable => true),
      QueryColumn.new(:end_date, :sortable => "#{Client.table_name}.end_date",:groupable => true),
      QueryColumn.new(:start_time, :sortable => "#{Client.table_name}.start_time",:groupable => true),
      QueryColumn.new(:status, :sortable => "#{Client.table_name}.status",:groupable => true),
  ]
